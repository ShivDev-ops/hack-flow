...
async function debugAuth(id, pass) {
  ...
  if (!isValid) {
      console.log("EXPECTED_HASH:", organizer.password_hash);
  }
}

async function run() {
    await debugAuth("SUPER-ADMIN", "SA_RECOVERY_2026");
    await debugAuth("HACK-ZYWM6", "i005p60b"); // From previous turn creation
}
